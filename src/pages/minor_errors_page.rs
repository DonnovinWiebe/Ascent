...
/// The page used to display minor errors.
#[must_use]
pub fn minor_errors_page<'a>(
    app: &'a App,
) -> Stack<'a, Signal> {
    stack![
        container(minor_errors_panel(app)).center(Fill),
        header(app, Vec::new()),
    ]
}

/// Displays the minor errors collected by the `App`.
fn minor_errors_panel<'a>(
    app: &'a App,
) -> Element<'a, Signal> {
    panel(
        app,
        ...
        PaddingSizes::Small, {
            column![
                ui_string(app, "Ascent has encountered some slight difficulties...", TextSizes::LargeHeading, MaterialColors::StrongText),
                ...
                panel(
                    ...
                    PaddingSizes::None, {
                        let mut errors = app.minor_errors.iter().map(|e| ui_string(app, e, TextSizes::SmallHeading, MaterialColors::StrongText)).collect::<Vec<_>>();
                ...
                spacer(Orientations::Vertical, Spacing::Large),
                dismiss_minor_errors_button(app)
            ]
            .align_x(Center)
...
/// A button that dismisses every minor error.
#[must_use]
fn dismiss_minor_errors_button<'a>(
    app: &'a App,
) -> Element<'a, Signal> {
    panel_button(
        app,
