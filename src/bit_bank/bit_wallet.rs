...
impl BitWallet {
    ...
    /// This is intended to be used when a new `Bit` is created from within the `App`.
    #[must_use]
    pub fn add_bit_from_raw_parts(&mut self, amount_string: &str, coin_value_string: &str, coin_value_currency_string: &str, date: Date) -> Schrod<()> {
        // the amount
        ...
        // the coin value
        let coin_value_result = Schrod::from_result(Decimal::from_str(coin_value_string), "Failed to convert coin_value_string to Decimal.", "BitWallet::add_bit_from_raw_parts()");
        if coin_value_result.is_fail() {
            return coin_value_result
                .convert("BitWallet::add_bit_from_raw_parts()")
                .fail("Failed to add Bit from raw parts.", "BitWallet::add_bit_from_raw_parts()")
        }
        let coin_value = coin_value_result.wont_fail("This is past an is_fail() guard clause.", "BitWallet::add_bit_from_raw_parts()");
        let coin_value_currency_result = Schrod::from_option(iso::find(&coin_value_currency_string.to_uppercase()), "Failed to convert coin_value_currency_string to Currency.", "BitWallet::add_bit_from_raw_parts()");
        ...
        let coin_value = Value::from_decimal(coin_value, coin_value_currency);
    ...
    #[must_use]
    pub fn edit_bit_with_raw_parts(&mut self, id: Uuid, amount_string: &str, coin_value_string: &str, coin_value_currency_string: &str, date: Date) -> Schrod<()> {
        // the amount
        let amount_result = Schrod::from_result(Decimal::from_str_exact(amount_string), "Failed to convert amount_string to Decimal!", "BitWallet::add_bit_from_raw_parts()");
        if amount_result.is_fail() {
            return amount_result
        ...
        let coin_value = coin_value_result.wont_fail("This is past an is_fail() guard clause.", "BitWallet::edit_bit_with_raw_parts()");
        let coin_value_currency_result = Schrod::from_option(iso::find(&coin_value_currency_string.to_uppercase()), "Failed to convert coin_value_currency_string to Currency.", "BitWallet::add_bit_from_raw_parts()");
        if coin_value_currency_result.is_fail() {
            return coin_value_currency_result
                .convert("BitWallet::edit_bit_with_raw_parts()")
